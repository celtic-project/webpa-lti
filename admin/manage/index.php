...
<div class="content_box">
    ...
    if ($resource_link->hasMembershipsService()) {
        ...
        if (lti_fetch_POST('do')) {
        ...
        } elseif (lti_fetch_POST('continue')) {
            #
            ...
            if ($members !== false) {
                ...
                foreach ($resource_link->groupSets as $set_id => $set) {
                    ...
                    } elseif (!empty($groups_map[$set_id])) {
                            ...
                            if ($user->isLearner()) {
                                ...
                                foreach ($user->groups as $group) {
                                    if (isset($resource_link->groups[$group]['set']) && ($resource_link->groups[$group]['set'] === $set_id)) {
                                        ...
                                        } elseif ($in_group != $groups_map[$set_id]['groups'][$group]) {
    ...
    ?>
                <form action="" method="post" onsubmit="onSubmitSyncForm()">
                    ...
                    #
                    if (
                        (count($to_add) > 0) || (count($to_update) > 0) || (count($to_update_role) > 0) || (count($users) > 0) ||
